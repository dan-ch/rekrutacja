    ...
    <main class="main">
        <section class="search">
            ...
            <div class="search__checkbox">
                    ...
                    <a type="radio" href="bike.php" class="link-reset search__checkbox-item bike"></a>
                    <a type="radio" href="truck.php" style="border-bottom: none" class="link-reset search__checkbox-item truck"></a>
                </div>
                    ...
                    <div class="input">
                        <select class="input-item" name="make" id="make" required>
                            ...
                            try {
                                ...
                                $marki = $baza->query("SELECT DISTINCT Marka FROM dostawcze")->fetchAll();
                ...
                <div class="search__atomic">
                    ...
                    <div class="input">
                        <select class="input-item" name="fuel" id="fuel">
                    ...
                    <input class="input" type="number" name="miejsca" min="0" max="250000" step="1" id="books"
                        placeholder="Ladownośc w kg">
                </div>
            ...
            if(isset($_GET['szukaj'])) {
                $mysql1 = "SELECT Id, Marka, Model, Rok_produkcji, Przebieg, Lokalizacja, Cena FROM dostawcze WHERE ";
                $mysql2 = "null";
                ...
                if(!empty($_GET["fuel"])) $mysql2 .= " AND Paliwo = '".$_GET["fuel"]."'";
                if(!empty($_GET["kraj"])) $mysql2 .= " AND Kraj = '".$_GET["kraj"]."'";
                if(!empty($_GET["miejsca"])) $mysql2 .= " AND Ladownosc = '".$_GET["miejsca"]."'";
            ...
            <?php
                if(!empty($zwrot))
                {
                    foreach($zwrot as $ogl)
                    {
                        ...
                        echo '<img src="assets/img/dostawcze/'.$ogl["Id"].'/0.jpg" alt="">';
                        echo '<div>';
                        echo '<a href="offertruck.php?id='.$ogl["Id"].'" class="link-reset">';
                ...
                {
                    $ile = $baza->query("SELECT count(*) FROM dostawcze")->fetchColumn();
                    if($ile > 4) $ile = 4;
                    $ids = $baza->query("SELECT Id FROM dostawcze")->fetchAll();
                    $losowe = $baza->query("SELECT Id, Marka, Model, Rok_produkcji, Przebieg, Lokalizacja, Cena FROM dostawcze")->fetchAll();
                    for($i=0; $i<$ile; $i++)
                    {
                      if(count($ids) > 4) $numer = rand(0, count($ids)-1);
                      else $numer = $i;
                        $losowe = $baza->query("SELECT Id, Marka, Model, Rok_produkcji, Przebieg, Lokalizacja, Cena FROM dostawcze WHERE Id = ".$ids[$numer]["Id"])->fetch();

                        echo '<article class="offer__item">';
                        echo '<img src="assets/img/dostawcze/'.$losowe["Id"].'/0.jpg" alt="">';
                        echo '<div>';
                        echo '<a href="offertruck.php?id='.$losowe["Id"].'" class="link-reset">';
