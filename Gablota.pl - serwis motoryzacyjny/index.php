        ...
        <section class="search">
            <form class="search-wrap" method="GET">
                <div class="search__checkbox">
                    <a type="radio" style="border-bottom: none" href="index.php" class="link-reset search__checkbox-item car "></a>
                    ...
                    <div class="input">
                        <select class="input-item" name="make" id="make" required>
                            ...
                            try {
                                ...
                                $marki = $baza->query("SELECT DISTINCT Marka FROM osobowe")->fetchAll();
                ...
                <div class="search__atomic">
                    ...
                    <div class="input">
                        <select class="input-item" name="nadwozie" id="type">
            ...
            if(isset($_GET['szukaj'])) {
                $mysql1 = "SELECT Id, Marka, Model, Rok_produkcji, Przebieg, Lokalizacja, Cena FROM osobowe WHERE ";
                $mysql2 = "null";
            ...
            <?php
                if(!empty($zwrot))
                {
                    foreach($zwrot as $ogl)
                    {
                        ...
                        echo '<img src="assets/img/osobowe/'.$ogl["Id"].'/0.jpg" alt="">';
                        echo '<div>';
                        echo '<a href="offer.php?id='.$ogl["Id"].'" class="link-reset">';
                ...
                {
                    $ile = $baza->query("SELECT count(*) FROM osobowe")->fetchColumn();
                    if($ile > 4) $ile = 4;
                    $ids = $baza->query("SELECT Id FROM osobowe")->fetchAll();
                    $losowe = $baza->query("SELECT Id, Marka, Model, Rok_produkcji, Przebieg, Lokalizacja, Cena FROM osobowe")->fetchAll();
                    for($i=0; $i<$ile; $i++)
                    {
                        if(count($ids) > 4) $numer = rand(0, count($ids)-1);
                        else $numer = $i;
                        $losowe = $baza->query("SELECT Id, Marka, Model, Rok_produkcji, Przebieg, Lokalizacja, Cena FROM osobowe WHERE Id = ".$ids[$numer]["Id"])->fetch();

                        echo '<article class="offer__item">';
                        echo '<img src="assets/img/osobowe/'.$losowe["Id"].'/0.jpg" alt="">';
                        echo '<div>';
                        echo '<a href="offer.php?id='.$losowe["Id"].'" class="link-reset">';
